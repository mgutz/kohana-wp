...
function bootstrap($app_path)  {
	...
	if (!realpath($app_path)) {
		throw new Kohana_Exception("Invalid application path.", $app_path);
	}
...
function kohana_activate()
{
	...
	add_option('kwp_placement_front_loader', $kohana_front_loader);
	add_option('kwp_placement_default_placement','replace');
	add_option('kwp_placement_process_all_uri', 1 );
	add_option('kwp_placement_system_path', WP_PLUGIN_DIR . '/kohana-wp/kohana/system/');
	add_option('kwp_placement_module_path', WP_PLUGIN_DIR . '/kohana-wp/kohana/modules/');
	add_option('kwp_placement_application_path', WP_PLUGIN_DIR . '/kohana-wp/kohana/application/');
	add_option('kwp_placement_bootstrap_path', '');
	add_option('kwp_placement_ext', '.php' );
	add_option('kwp_placement_modules', '');
	add_option('kwp_placement_default_controller', '' );
	add_option('kwp_placement_default_action', '' );
	add_option('kwp_placement_default_id', '' );
	add_option('kwp_placement_front_loader_in_nav', 0);
	add_option('kwp_placement_page_template', '');
}
...
function should_kohana_run()
{
	...
	if( ! get_option('kwp_system_path') && ! get_option('kwp_application_path') ){
			return false;
	}
...
function kohana_request_filter($request)
{
	...
	// Request is for our wordpress kohana front loader	
	} else if ($requested_post_id == get_option('kwp_front_loader')) {
		...
		if (get_option('kwp_route::'.$requested_post_id) != '') {
			$arr = explode( '::', get_option('kwp_route::'.$requested_post_id) );
...
function kohana_parse_request()
{
	...
	list($app, $controller, $action, $args) = explode('/', $kr, 4);
...
function kohana_title_filter($title)
{
	...
	global $wp;
	global $post;
	if( $wp->kohana->title && $title == $post->post_title && $post->ID == get_option('kwp_front_loader') ){
...
function execute_request($kr) {
	...
	bootstrap($app_path);
	return Request::factory($parts[1])->execute();
}
